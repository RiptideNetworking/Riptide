...
namespace Riptide.Transports.NanoSockets
{
    ...
    public class NanoSocketsClient : NanoSocketsPeer, IClient
    {
        /// <inheritdoc/>
        ...
        private NanoSocketsConnection udpConnection;

        /// <inheritdoc/>
        public NanoSocketsClient(SocketMode mode = SocketMode.Both, int socketBufferSize = DefaultSocketBufferSize) : base(mode, socketBufferSize) { }
        ...
        public unsafe bool Connect(string hostAddress, out Connection connection, out string connectError)
        {
            ...
            ip = mode == SocketMode.IPv4Only ? ip : ip.MapToIPv6();
            NanoSocketsIPEndPoint endPoint;
            nanosockets_set_ip(&endPoint, ip.ToString());
            endPoint.port = port;
            
            connection = udpConnection = new NanoSocketsConnection(endPoint, this);
